...
// Delete a religious place
export const deleteReligiousPlace = protectedProcedure
  .input(z.string())
  .mutation(async ({ ctx, input }) => {
    ...
    if (ctx.user.role !== "superadmin") {
      throw new TRPCError({
        code: "UNAUTHORIZED",
        message: "Only administrators can delete religious places",
      });
    }

    try {
      // Get all media associated with this religious place
      ...
      // Delete the religious place
      await ctx.db.delete(religiousPlace).where(eq(religiousPlace.id, input));

      return { success: true };
    } catch (error) {
      console.error("Error deleting religious place:", error);
