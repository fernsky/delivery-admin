...
// Get religious place by slug
export const getReligiousPlaceBySlug = publicProcedure
  .input(z.string())
  .query(async ({ ctx, input }) => {
    try {
      const placeData = await ctx.db
        ...
        .from(religiousPlace)
        .where(eq(religiousPlace.slug, input))
        .limit(1);
      ...
      console.error("Error fetching religious place by slug:", error);
