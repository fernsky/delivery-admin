...
export const deleteHistoricalSite = protectedProcedure
  .input(z.string())
  .mutation(async ({ ctx, input }) => {
    ...
    if (ctx.user.role !== "superadmin") {
      throw new TRPCError({
        code: "UNAUTHORIZED",
        message: "Only administrators can delete historical sites",
      });
    }

    try {
      // Get all media associated with this historical site
      ...
      // Delete the historical site
      await ctx.db.delete(historicalSite).where(eq(historicalSite.id, input));
      ...
      throw new TRPCError({
        ...
        message: "Failed to delete historical site",
      });
    }
  });
