...
// Get cultural heritage by slug
export const getCulturalHeritageBySlug = publicProcedure
  .input(z.string())
  .query(async ({ ctx, input }) => {
    try {
      const heritageData = await ctx.db
        ...
        .where(eq(culturalHeritage.slug, input))
        .limit(1);
      ...
      console.error("Error fetching cultural heritage by slug:", error);
