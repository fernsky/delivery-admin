...
// Delete a cultural heritage item
export const deleteCulturalHeritage = protectedProcedure
  .input(z.string())
  .mutation(async ({ ctx, input }) => {
    ...
    if (ctx.user.role !== "superadmin") {
      throw new TRPCError({
        code: "UNAUTHORIZED",
        message: "Only administrators can delete cultural heritage items",
      });
    }

    try {
      // Get all media associated with this cultural heritage
      ...
      // Delete the cultural heritage item
      await ctx.db
        .delete(culturalHeritage)
        .where(eq(culturalHeritage.id, input));

      return { success: true };
    } catch (error) {
      console.error("Error deleting cultural heritage:", error);
      if (error instanceof TRPCError) throw error;

      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "Failed to delete cultural heritage",
      });
    }
  });
