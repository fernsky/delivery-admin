  ...
  .query(async ({ ctx, input }) => {
    try {
      ...
      try {
        ...
        if (input?.wardId) {
          conditions.push(
            eq(wardAgeGenderWiseAbsentee.wardId, input.wardId),
          );
        }

        if (input?.ageGroup) {
          conditions.push(eq(wardAgeGenderWiseAbsentee.ageGroup, input.ageGroup));
        }
        ...
        data = await queryWithFilters.orderBy(
          wardAgeGenderWiseAbsentee.wardId,
      ...
      console.error("Error fetching ward-age-gender-wise absentee data:", error);
      throw new TRPCError({
...
export const getWardAgeGenderWiseAbsenteeByWard = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    const data = await ctx.db
      ...
      .where(eq(wardAgeGenderWiseAbsentee.wardId, input.wardId))
      .orderBy(
        wardAgeGenderWiseAbsentee.ageGroup,
        wardAgeGenderWiseAbsentee.gender
      );
