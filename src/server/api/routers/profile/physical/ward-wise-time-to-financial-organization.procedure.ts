...
import { wardWiseTimeToFinancialOrganization } from "@/server/db/schema/profile/physical/ward-wise-time-to-financial-organization";
...
import {
  ...
  TimeToFinancialOrganizationTypeEnum,
  ...
  .query(async ({ ctx, input }) => {
    try {
      ...
      try {
        ...
        if (input?.timeToFinancialOrganization) {
          conditions.push(
            eq(
              wardWiseTimeToFinancialOrganization.timeToFinancialOrganization,
              input.timeToFinancialOrganization,
            ),
          );
        }
      ...
      if (!data || data.length === 0) {
        const acmeSql = sql`
          ...
          ORDER BY 
            ward_number, time_to_financial_organization
        `;
        ...
        if (acmeResult && Array.isArray(acmeResult) && acmeResult.length > 0) {
          ...
          data = acmeResult.map((row) => ({
            ...
            timeToFinancialOrganization: row.time_to_financial_organization,
          ...
          if (input?.timeToFinancialOrganization) {
            data = data.filter(
              (item) =>
                item.timeToFinancialOrganization ===
                input.timeToFinancialOrganization,
            );
          }
        }
      ...
      console.error(
        "Error fetching ward-wise time to financial organization data:",
        error,
      );
      throw new TRPCError({
...
export const getWardWiseTimeToFinancialOrganizationByWard = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    const data = await ctx.db
      ...
      .orderBy(wardWiseTimeToFinancialOrganization.timeToFinancialOrganization);
...
export const createWardWiseTimeToFinancialOrganization = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    if (existing.length > 0) {
      throw new TRPCError({
        code: "CONFLICT",
        message: `Data for Ward Number ${input.wardNumber} and time category ${input.timeToFinancialOrganization} already exists`,
      });
    }
    ...
    await ctx.db.insert(wardWiseTimeToFinancialOrganization).values({
      ...
      timeToFinancialOrganization: input.timeToFinancialOrganization,
...
export const getWardWiseTimeToFinancialOrganizationSummary =
  publicProcedure.query(async ({ ctx }) => {
    try {
      ...
      console.error(
        "Error in getWardWiseTimeToFinancialOrganizationSummary:",
        error,
      );
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message:
          "Failed to retrieve ward-wise time to financial organization summary",
      });
    }
  });
