...
// Get farm by slug
export const getFarmBySlug = publicProcedure
  .input(z.string())
  .query(async ({ ctx, input }) => {
    try {
      const farmData = await ctx.db
        .select({
          farmItem: {
            ...
            verificationDate: farm.verificationDate,
      ...
      console.error("Error fetching farm by slug:", error);
