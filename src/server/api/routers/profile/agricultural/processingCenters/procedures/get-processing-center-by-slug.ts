...
// Get processing center by slug
export const getProcessingCenterBySlug = publicProcedure
  .input(z.string())
  .query(async ({ ctx, input }) => {
    try {
      const centerData = await ctx.db
        ...
        .where(eq(agriProcessingCenter.slug, input))
        .limit(1);
      ...
      console.error("Error fetching processing center by slug:", error);
