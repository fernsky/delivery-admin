...
// Delete a fish farm
export const deleteFishFarm = protectedProcedure
  .input(z.string())
  .mutation(async ({ ctx, input }) => {
    ...
    if (ctx.user.role !== "superadmin") {
      throw new TRPCError({
        code: "UNAUTHORIZED",
        message: "Only administrators can delete fish farms",
      });
    }

    try {
      // Get all media associated with this fish farm
      ...
      // Delete the fish farm
      await ctx.db.delete(fishFarm).where(eq(fishFarm.id, input));
      ...
      throw new TRPCError({
        ...
        message: "Failed to delete fish farm",
      });
    }
  });
