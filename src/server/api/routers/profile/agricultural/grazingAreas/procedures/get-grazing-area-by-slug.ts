...
// Get grazing area by slug
export const getGrazingAreaBySlug = publicProcedure
  .input(z.string())
  .query(async ({ ctx, input }) => {
    try {
      const grazingAreaData = await ctx.db
        ...
        .from(grazingArea)
        .where(eq(grazingArea.slug, input))
        .limit(1);
      ...
      console.error("Error fetching grazing area by slug:", error);
