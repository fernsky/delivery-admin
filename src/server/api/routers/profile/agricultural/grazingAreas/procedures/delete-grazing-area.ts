...
// Delete a grazing area
export const deleteGrazingArea = protectedProcedure
  .input(z.string())
  .mutation(async ({ ctx, input }) => {
    ...
    if (ctx.user.role !== "superadmin") {
      throw new TRPCError({
        code: "UNAUTHORIZED",
        message: "Only administrators can delete grazing areas",
      });
    }

    try {
      // Get all media associated with this grazing area
      ...
      // Delete the grazing area
      await ctx.db.delete(grazingArea).where(eq(grazingArea.id, input));

      return { success: true };
    } catch (error) {
      console.error("Error deleting grazing area:", error);
      if (error instanceof TRPCError) throw error;

      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "Failed to delete grazing area",
      });
    }
  });
