...
export const getAll = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    if (filters) {
      ...
      if (filters.wardNo) {
        filterConditions.push(eq(productIndividual.wardNo, filters.wardNo));
      }
      if (filters.familyId) {
        filterConditions.push(eq(productIndividual.familyId, filters.familyId));
      }
      if (filters.gender) {
        filterConditions.push(eq(productIndividual.gender, filters.gender));
      }
      if (filters.ageRange) {
        if (filters.ageRange.min !== undefined) {
          filterConditions.push(
            sql`${productIndividual.age} >= ${filters.ageRange.min}`,
          );
        }
        if (filters.ageRange.max !== undefined) {
          filterConditions.push(
            sql`${productIndividual.age} <= ${filters.ageRange.max}`,
          );
        }
      }
...
export const getById = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    const individual = await ctx.db
      .select()
      .from(productIndividual)
      .where(eq(productIndividual.id, input.id))
      .limit(1);
...
export const getStats = publicProcedure.query(async ({ ctx }) => {
  const stats = await ctx.db
    .select({
      ...
      averageAge: sql<number>`ROUND(AVG(${productIndividual.age}))::integer`,
    })
    .from(productIndividual);
