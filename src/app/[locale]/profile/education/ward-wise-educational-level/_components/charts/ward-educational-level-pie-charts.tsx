...
interface WardEducationalLevelPieChartsProps {
...
export default function WardEducationalLevelPieCharts({
  wardWiseData,
  EDUCATIONAL_LEVEL_GROUPS,
}: WardEducationalLevelPieChartsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {wardWiseData.map((wardData) => {
        ...
        const educationGroups = Object.values(EDUCATIONAL_LEVEL_GROUPS)
          .map(group => ({
            name: group.name,
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {educationGroups.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.color}
                      />
