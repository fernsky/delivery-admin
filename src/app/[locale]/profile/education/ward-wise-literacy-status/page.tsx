...
export async function generateMetadata(): Promise<Metadata> {
  try {
    ...
    const wardGroups = wardWiseLiteracyStatusData.reduce((acc: any, curr: any) => {
    ...
    const bothReadingWritingPercentage = ((bothReadingWritingTotal / totalPopulation) * 100).toFixed(2);
    const illiteratePercentage = ((illiterateTotal / totalPopulation) * 100).toFixed(2);
    ...
    Object.entries(wardGroups).forEach(([ward, data]: [string, any]) => {
      const wardTotalPopulation = data.reduce((sum: number, item: any) => sum + item.population, 0);
      ...
      const literacyPercentage = (literatePopulation / wardTotalPopulation) * 100;
    ...
    return {
      title: "साक्षरताको अवस्था | खजुरा गाउँपालिका डिजिटल प्रोफाइल",
...
const toc = [
  ...
  { level: 2, text: "साक्षरताको अवस्थाको वितरण", slug: "distribution-of-literacy-status" },
  { level: 2, text: "वडा अनुसार साक्षरताको अवस्था", slug: "ward-wise-literacy-status" },
  { level: 2, text: "साक्षरता विश्लेषण", slug: "literacy-analysis" },
  { level: 2, text: "साक्षरता प्रवर्द्धन रणनीति", slug: "literacy-promotion-strategy" },
];
...
export default async function WardWiseLiteracyStatusPage() {
  ...
  const bothReadingWritingPercentage = parseFloat(((bothReadingWritingTotal / totalPopulation) * 100).toFixed(2));
  const readingOnlyPercentage = parseFloat(((readingOnlyTotal / totalPopulation) * 100).toFixed(2));
  const illiteratePercentage = parseFloat(((illiterateTotal / totalPopulation) * 100).toFixed(2));
  ...
  const wardWiseData = wardNumbers.map((wardNumber) => {
    ...
    const bothReadingWriting = wardData.find((item: any) => item.literacyType === 'BOTH_READING_AND_WRITING')?.population || 0;
    const readingOnly = wardData.find((item: any) => item.literacyType === 'READING_ONLY')?.population || 0;
    const illiterate = wardData.find((item: any) => item.literacyType === 'ILLITERATE')?.population || 0;
    
    return {
      ward: `वडा ${wardNumber}`,
      wardNumber,
      [LITERACY_STATUS_TYPES.BOTH_READING_AND_WRITING.name]: bothReadingWriting,
      ...
      bothReadingWritingPercent: totalWardPopulation > 0 ? (bothReadingWriting / totalWardPopulation) * 100 : 0,
      readingOnlyPercent: totalWardPopulation > 0 ? (readingOnly / totalWardPopulation) * 100 : 0,
      illiteratePercent: totalWardPopulation > 0 ? (illiterate / totalWardPopulation) * 100 : 0,
    };
  }).filter(Boolean);
  ...
  const wardWiseAnalysis = wardWiseData.map((ward: any) => {
    return {
      ...
      bothReadingWritingPopulation: ward?.[LITERACY_STATUS_TYPES.BOTH_READING_AND_WRITING.name] || 0,
      readingOnlyPopulation: ward?.[LITERACY_STATUS_TYPES.READING_ONLY.name] || 0,
      illiteratePopulation: ward?.[LITERACY_STATUS_TYPES.ILLITERATE.name] || 0,
      ...
      literacyPercent: (ward?.bothReadingWritingPercent || 0) + (ward?.readingOnlyPercent || 0),
    };
        ...
        <section>
          <div className="relative rounded-lg overflow-hidden mb-8">
            <Image
              ...
              width={1200}
              height={400}
              alt="साक्षरताको अवस्था - खजुरा गाउँपालिका (Literacy Status - Khajura Rural Municipality)"
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none">
            <h1 className="scroll-m-20 tracking-tight mb-6">
              खजुरा गाउँपालिकामा साक्षरताको अवस्था
            </h1>
            ...
            <p>
              साक्षरता एक आधारभूत मानव अधिकार हो र यो व्यक्तिगत, सामाजिक र आर्थिक विकासको महत्वपूर्ण 
              आधारशिला हो। यसले नागरिकको जीवनस्तरमा सुधार ल्याउनुका साथै राष्ट्रिय विकासमा पनि प्रत्यक्ष योगदान 
              पुर्‍याउँछ। यस खण्डमा खजुरा गाउँपालिकाको विभिन्न वडाहरूमा नागरिकहरूको साक्षरताको अवस्थाको 
              विश्लेषण प्रस्तुत गरिएको छ।
            </p>
            <p>
              खजुरा गाउँपालिकामा कुल {localizeNumber(totalPopulation.toLocaleString(), "ne")} जनसंख्यामध्ये 
              {localizeNumber(bothReadingWritingPercentage.toFixed(2), "ne")}% अर्थात् {localizeNumber(bothReadingWritingTotal.toLocaleString(), "ne")} 
              जनाले पढ्न र लेख्न जान्दछन्, {localizeNumber(readingOnlyPercentage.toFixed(2), "ne")}% 
              अर्थात् {localizeNumber(readingOnlyTotal.toLocaleString(), "ne")} जनाले पढ्न मात्र जान्दछन्, र 
              {localizeNumber(illiteratePercentage.toFixed(2), "ne")}% अर्थात् {localizeNumber(illiterateTotal.toLocaleString(), "ne")} 
            ...
            <p>
              खजुरा गाउँपालिकामा साक्षरताको अवस्थाको वितरण निम्नानुसार रहेको छ:
            </p>
          </div>
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none mt-8">
            ...
            <p>
              खजुरा गाउँपालिकामा साक्षरताको अवस्थाको विश्लेषण गर्दा, समग्रमा 
              {localizeNumber((bothReadingWritingPercentage + readingOnlyPercentage).toFixed(2), "ne")}% जनसंख्या 
              साक्षर रहेका छन्, जसमध्ये {localizeNumber(bothReadingWritingPercentage.toFixed(2), "ne")}% ले पढ्न र लेख्न जान्दछन्।
              वडागत रूपमा हेर्दा वडा नं. {localizeNumber(bestLiteracyWard.wardNumber.toString(), "ne")} मा 
              सबैभन्दा राम्रो साक्षरता दर {localizeNumber(bestLiteracyWard.literacyPercent.toFixed(2), "ne")}% रहेको छ।
            </p>
            ...
            <p>
              खजुरा गाउँपालिकामा साक्षरताको अवस्थाको तथ्याङ्क विश्लेषणबाट निम्न रणनीतिहरू 
              अवलम्बन गर्न सकिन्छ:
            </p>
            ...
            <div className="pl-6 space-y-4">
              <div className="flex">
                ...
                <div>
                  <strong>साक्षरता अभियान सञ्चालन:</strong> {localizeNumber(illiteratePercentage.toFixed(2), "ne")}% 
                  जनसंख्या अझै पनि निरक्षर रहेकाले त्यस्ता क्षेत्रमा वृहत साक्षरता अभियान सञ्चालन गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">२.</span>
                <div>
                  <strong>प्रौढ शिक्षा कार्यक्रम:</strong> विशेषगरी वडा {localizeNumber(worstLiteracyWard.wardNumber.toString(), "ne")} जस्ता 
                  निरक्षरता दर बढी भएका क्षेत्रमा प्रौढ शिक्षा केन्द्रहरूको स्थापना र सञ्चालन गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">३.</span>
                <div>
                  <strong>विद्यालय भर्ना अभियान:</strong> बालबालिकाहरूलाई विद्यालय भर्ना गर्न प्रोत्साहन गर्ने 
                  र विद्यालय छाड्ने दरलाई न्यूनीकरण गर्ने रणनीतिहरू अवलम्बन गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">४.</span>
                <div>
                  <strong>गुणस्तरीय शिक्षा:</strong> विद्यालयहरूमा पठनपाठनको गुणस्तर सुधार गरी विद्यार्थीहरूमा 
                  पढ्ने र लेख्ने क्षमताको प्रभावकारी विकास गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">५.</span>
                <div>
                  <strong>प्राविधिक शिक्षा:</strong> साक्षरता सँगै प्राविधिक सीप विकासका कार्यक्रमहरू सञ्चालन गर्ने, 
                  जसले आर्थिक आत्मनिर्भरतामा सहयोग पुर्‍याउँछ।
                </div>
              </div>
            </div>

            <p className="mt-6">
              यसरी खजुरा गाउँपालिकामा साक्षरताको अवस्थाको विश्लेषणले पालिकामा 
              शैक्षिक विकासको अवस्था र भविष्यको शैक्षिक नीति निर्माणमा महत्वपूर्ण भूमिका खेल्दछ। यसका लागि 
              वडागत विशेषताहरूलाई ध्यानमा राखी शिक्षा क्षेत्रको विस्तारका कार्यक्रमहरू तर्जुमा गर्नु आवश्यक देखिन्छ।
            </p>
          </div>
        </section>
      </div>
