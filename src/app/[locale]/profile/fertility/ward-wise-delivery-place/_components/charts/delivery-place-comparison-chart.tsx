...
export default function DeliveryPlaceComparisonChart({
  ...
  const chartData = wardInstitutionalPercentages.map((ward) => ({
    name: `वडा ${ward.wardNumber}`,
    "InstitutionalDelivery": ward.percentage,
  })).sort((a, b) => 
    b["InstitutionalDelivery"] - a["InstitutionalDelivery"]
  );

  // Calculate average institutional delivery rate
  const avgInstitutionalRate =
    wardInstitutionalPercentages.reduce((sum, ward) => sum + ward.percentage, 0) / wardInstitutionalPercentages.length;
