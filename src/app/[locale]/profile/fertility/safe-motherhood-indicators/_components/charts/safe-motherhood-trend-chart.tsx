...
interface SafeMotherhoodTrendChartProps {
  trendData: Array<{
    indicator: string;
...
export default function SafeMotherhoodTrendChart({
  trendData,
  indicatorLabels,
}: SafeMotherhoodTrendChartProps) {
  ...
  const lineColors = ["#ec4899", "#8b5cf6", "#a855f7", "#06b6d4"];
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-background p-3 border shadow-sm rounded-md">
          <p className="font-medium mb-2">{label} वर्ष</p>
