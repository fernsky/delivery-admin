...
export async function generateMetadata(): Promise<Metadata> {
  try {
    ...
    Object.entries(facilityGroups).forEach(([facility, items]: [string, any]) => {
      ...
      const facilityTotal = items.reduce((sum: number, item: any) => sum + item.households, 0);
    ...
    const noneTotal = facilityGroups.NONE?.reduce((sum: number, item: any) => sum + item.households, 0) || 0;
    
    // Find most common facilities
    const sortedFacilities = Object.entries(facilityTypeTotals)
      .sort(([, a]: [string, number], [, b]: [string, number]) => b - a);
    
    const topFacilities = sortedFacilities.slice(0, 3).map(([facility]) => facility);
    
    const mobilePercentage = facilityTypeTotals.MOBILE_PHONE ? 
      ((facilityTypeTotals.MOBILE_PHONE / uniqueHouseholds.size) * 100).toFixed(2) : "0.00";
    const televisionPercentage = facilityTypeTotals.TELEVISION ? 
      ((facilityTypeTotals.TELEVISION / uniqueHouseholds.size) * 100).toFixed(2) : "0.00";
    const internetPercentage = facilityTypeTotals.INTERNET ? 
      ((facilityTypeTotals.INTERNET / uniqueHouseholds.size) * 100).toFixed(2) : "0.00";

    // Create rich keywords
    const keywordsNP = [
      "खजुरा गाउँपालिका घरायसी सुविधा",
    ...
    // Create description
    const descriptionNP = `खजुरा गाउँपालिकामा घरायसी सुविधाको प्रयोगको विश्लेषण। ${localizeNumber(mobilePercentage, "ne")}% घरहरूमा मोबाइल फोन, ${localizeNumber(televisionPercentage, "ne")}% घरहरूमा टेलिभिजन र ${localizeNumber(internetPercentage, "ne")}% घरहरूमा इन्टरनेट सुविधा रहेको छ।`;
    ...
    return {
      title: "घरायसी सुविधाको प्रयोगको अवस्था | खजुरा गाउँपालिका डिजिटल प्रोफाइल",
...
const toc = [
  ...
  { level: 2, text: "घरायसी सुविधाको वितरण", slug: "distribution-of-household-facilities" },
  { level: 2, text: "वडा अनुसार घरायसी सुविधाको प्रयोग", slug: "ward-wise-facilities-usage" },
  { level: 2, text: "घरायसी सुविधा प्रयोगको विश्लेषण", slug: "facilities-usage-analysis" },
  { level: 2, text: "घरायसी सुविधा विस्तार रणनीति", slug: "facilities-expansion-strategy" },
];
...
export default async function WardWiseFacilitiesPage() {
  ...
  try {
    summaryData =
      await api.profile.physical.wardWiseFacilities.summary.query();
  } catch (error) {
  ...
  Object.keys(facilityTypeTotals).forEach(facility => {
    facilityTypePercentages[facility] = parseFloat(((facilityTypeTotals[facility] / approximateUniqueHouseholds) * 100).toFixed(2));
  });
  ...
  const wardWiseData = wardNumbers.map((wardNumber) => {
    ...
    const mobilePhoneItem = wardData.find((item: any) => item.facility === "MOBILE_PHONE");
    ...
    Object.keys(FACILITY_CATEGORIES).forEach(categoryKey => {
      const category = FACILITY_CATEGORIES[categoryKey as keyof typeof FACILITY_CATEGORIES];
      const facilityItem = wardData.find((item: any) => item.facility === categoryKey);
      wardFacilityCategories[category.name] = facilityItem ? facilityItem.households : 0;
    });
  ...
  const facilityCategoriesForAnalysis = {
    communication: ["MOBILE_PHONE", "RADIO", "TELEVISION", "INTERNET", "DAILY_NATIONAL_NEWSPAPER_ACCESS"],
    transportation: ["BICYCLE", "MOTORCYCLE", "CAR_JEEP"],
    appliances: ["REFRIGERATOR", "WASHING_MACHINE", "AIR_CONDITIONER", "ELECTRICAL_FAN", "MICROWAVE_OVEN"],
    digital: ["COMPUTER", "INTERNET", "MOBILE_PHONE"],
  };

  // Calculate category totals and percentages
  const categoryStats: Record<string, {total: number, percentage: number}> = {};
  Object.entries(facilityCategoriesForAnalysis).forEach(([category, facilityList]) => {
    const total = facilityList.reduce((sum, facility) => sum + (facilityTypeTotals[facility] || 0), 0);
    const percentage = (total / (facilityList.length * approximateUniqueHouseholds)) * 100;
  ...
  const bestDigitalWard = [...wardDigitalAccess].sort((a, b) => b.score - a.score)[0];
  const worstDigitalWard = [...wardDigitalAccess].sort((a, b) => a.score - b.score)[0];
        ...
        <section>
          <div className="relative rounded-lg overflow-hidden mb-8">
            <Image
              ...
              width={1200}
              height={400}
              alt="घरायसी सुविधाको प्रयोगको अवस्था - खजुरा गाउँपालिका (Household Facilities Usage - Khajura Rural Municipality)"
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none">
            <h1 className="scroll-m-20 tracking-tight mb-6">
              खजुरा गाउँपालिकामा घरायसी सुविधाको प्रयोगको अवस्था
            </h1>
            ...
            <p>
              घरायसी सुविधाहरूको उपलब्धता र प्रयोगले समुदायको जीवनस्तर, आधुनिकीकरण र 
              सामाजिक-आर्थिक विकासको अवस्थालाई प्रतिविम्बित गर्दछ। यी सुविधाहरूको पहुँचले
              परिवारको दैनिक जीवन, शिक्षा, सूचना प्रविधिको प्रयोग, यातायात र आरामदायी जीवनयापनमा प्रत्यक्ष प्रभाव पार्दछ।
              यस खण्डमा खजुरा गाउँपालिकाको विभिन्न वडाहरूमा प्रयोग हुने घरायसी सुविधाहरूको विस्तृत विश्लेषण प्रस्तुत गरिएको छ।
            </p>
            <p>
              खजुरा गाउँपालिकामा अनुमानित कुल {localizeNumber(approximateUniqueHouseholds.toLocaleString(), "ne")} घरधुरी मध्ये 
              {localizeNumber(facilityTypePercentages.MOBILE_PHONE?.toFixed(2) || "0.00", "ne")}% घरहरूमा मोबाइल फोनको पहुँच,
              {localizeNumber(facilityTypePercentages.TELEVISION?.toFixed(2) || "0.00", "ne")}% घरहरूमा टेलिभिजन, र 
              {localizeNumber(facilityTypePercentages.INTERNET?.toFixed(2) || "0.00", "ne")}% घरहरूमा इन्टरनेट सुविधा रहेको छ।
            </p>
            ...
            <p>
              खजुरा गाउँपालिकामा घरायसी सुविधाहरूको प्रयोगको वितरण निम्नानुसार रहेको छ:
            </p>
          </div>
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none mt-8">
            <h2 id="facilities-usage-analysis" className="scroll-m-20 border-b pb-2">
              घरायसी सुविधाको प्रयोगको विश्लेषण
            </h2>
            <p>
              खजुरा गाउँपालिकामा घरायसी सुविधाहरूको प्रयोगको विश्लेषण गर्दा, समग्रमा सञ्चार सम्बन्धी सुविधाहरू
              (मोबाइल फोन, टेलिभिजन, इन्टरनेट) को पहुँच 
              {localizeNumber(categoryStats.communication.percentage.toFixed(2), "ne")}% रहेको देखिन्छ।
              वडागत रूपमा हेर्दा वडा नं. {localizeNumber(bestDigitalWard.wardNumber.toString(), "ne")} मा 
              डिजिटल सुविधाको पहुँच सबैभन्दा उच्च रहेको छ, जहाँ {localizeNumber(bestDigitalWard.score.toFixed(2), "ne")}% 
              पहुँच रहेको देखिन्छ।
            </p>
            ...
            <p>
              खजुरा गाउँपालिकामा घरायसी सुविधाको प्रयोगको तथ्याङ्क विश्लेषणबाट निम्न रणनीतिहरू 
              अवलम्बन गर्न सकिन्छ:
            </p>
            ...
            <div className="pl-6 space-y-4">
              <div className="flex">
                ...
                <div>
                  <strong>इन्टरनेट पहुँच विस्तार कार्यक्रम:</strong> {localizeNumber((100 - facilityTypePercentages.INTERNET || 0).toFixed(2), "ne")}% 
                  घरधुरीहरूमा अझै इन्टरनेट पहुँच नभएकोले, विशेषगरी वडा नं. {localizeNumber(worstDigitalWard.wardNumber.toString(), "ne")} मा 
                  इन्टरनेट पहुँच विस्तार गर्न सार्वजनिक वाइफाई हट-स्पट स्थापना र ब्रोडब्यान्ड विस्तारमा जोड दिने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">२.</span>
                <div>
                  <strong>डिजिटल साक्षरता कार्यक्रम:</strong> विशेषगरी कम्प्युटरको पहुँच कम भएका वडाहरूमा डिजिटल 
                  साक्षरता कक्षा सञ्चालन गरी आधारभूत कम्प्युटर सीप विकास गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">३.</span>
                <div>
                  <strong>सामुदायिक सूचना केन्द्र स्थापना:</strong> प्रत्येक वडामा एउटा सामुदायिक डिजिटल सूचना केन्द्र स्थापना गरी
              ...
              <div className="flex">
                ...
                <div>
                  <strong>नवीकरणीय ऊर्जा प्रवर्द्धन:</strong> सौर्य ऊर्जा प्रणालीको प्रयोग बढाएर विद्युतीय उपकरणहरूको प्रयोगमा वृद्धि गर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">५.</span>
                <div>
                  <strong>घरायसी उपकरण सहुलियत कार्यक्रम:</strong> आर्थिक रूपमा कमजोर परिवारहरूका लागि आवश्यक
                  घरायसी उपकरण (जस्तै: इलेक्ट्रिक पंखा, मोबाइल फोन) खरिदमा सुलभ ऋण वा अनुदान प्रदान गर्ने।
                </div>
              </div>
            </div>

            <p className="mt-6">
              यसरी खजुरा गाउँपालिकामा घरायसी सुविधाहरूको प्रयोगको विश्लेषणले पालिकाको सामाजिक-आर्थिक स्थिति र 
              जीवनस्तरको अवस्था बुझ्न मद्दत गर्दछ। आधुनिक घरायसी सुविधाहरूको पहुँचमा वृद्धि गर्न लक्षित कार्यक्रम 
              संचालन गरेर नागरिकहरूको सूचना, शिक्षा र स्वास्थ्यमा सुधार ल्याउन सकिन्छ, जसले समग्र पालिकाको 
              विकासमा योगदान पुर्‍याउनेछ।
            </p>
          </div>
        </section>
      </div>
