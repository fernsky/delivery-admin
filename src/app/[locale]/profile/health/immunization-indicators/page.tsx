...
export async function generateMetadata(): Promise<Metadata> {
  try {
    ...
    const latestFiscalYear = immunizationFiscalYearOptions[0].value as ImmunizationFiscalYear;
    const immunizationData = await api.profile.health.immunizationIndicators.getAll.query({
      fiscalYear: latestFiscalYear,
    });
    
    const municipalityName = "खजुरा गाउँपालिका"; // Khajura Rural Municipality
    const fiscalYearLabel = immunizationFiscalYearOptions.find(
      opt => opt.value === latestFiscalYear
    )?.label || latestFiscalYear;

    // Find key coverage indicators for description
    const dptHib3Data = immunizationData.find(item => item.indicator === "DPT_HEPB_HIB3_COVERAGE");
    const measlesRubella1Data = immunizationData.find(item => item.indicator === "MEASLES_RUBELLA1_COVERAGE");
    const fullyImmunizedData = immunizationData.find(item => item.indicator === "FULLY_IMMUNIZED_NIP_SCHEDULE");
    ...
    // Create description
    const descriptionNP = `खजुरा गाउँपालिकामा आधारभूत खोप सेवाको विश्लेषण। आर्थिक वर्ष ${fiscalYearLabel} मा पूर्ण खोप कभरेज ${localizeNumber(fullyImmunizedCoverage.toFixed(1), "ne")}%, DPT-HepB-Hib3 कभरेज ${localizeNumber(dptHib3Coverage.toFixed(1), "ne")}%, र दादुरा-रुबेला कभरेज ${localizeNumber(measlesRubella1Coverage.toFixed(1), "ne")}% रहेको छ।`;
    ...
    return {
      title: "खोप सेवा र कभरेज | खजुरा गाउँपालिका डिजिटल प्रोफाइल",
      description: "खजुरा गाउँपालिकामा आधारभूत खोप सेवा र कभरेजको अवस्था र विश्लेषण।",
    };
  }
}
...
const toc = [
  ...
  { level: 2, text: "प्रमुख खोप कभरेज सूचकहरू", slug: "key-immunization-coverage-indicators" },
  { level: 2, text: "खोप कभरेज प्रवृत्ति", slug: "immunization-coverage-trends" },
  { level: 2, text: "खोप ड्रपआउट दर", slug: "vaccine-dropout-rates" },
  { level: 2, text: "खोप खेर जाने दर", slug: "vaccine-wastage-rates" },
  { level: 2, text: "खोप सेवा सुधार रणनीति", slug: "immunization-service-improvement-strategy" },
];
...
export default async function ImmunizationIndicatorsPage() {
  ...
  const availableFiscalYears = [...immunizationFiscalYearOptions].map(o => o.value as ImmunizationFiscalYear);
  ...
  const latestImmunizationData = await api.profile.health.immunizationIndicators.getAll.query({
    fiscalYear: latestFiscalYear,
  });

  // Fetch previous year's data for comparison
  const previousImmunizationData = await api.profile.health.immunizationIndicators.getAll.query({
  ...
  for (let i = 0; i < Math.min(3, availableFiscalYears.length); i++) {
    const yearData = await api.profile.health.immunizationIndicators.getAll.query({
  ...
  const fiscalYearLabels = immunizationFiscalYearOptions.reduce<Record<string, string>>(
    (acc, option) => {
  ...
  const indicatorLabels = immunizationIndicatorOptions.reduce<Record<string, string>>(
    (acc, option) => {
      acc[option.value] = option.label;
      return acc;
    },
  ...
  const coverageData = latestImmunizationData.filter(
    item => coverageIndicators.includes(item.indicator)
  ).sort((a, b) => {
  ...
  const dropoutData = latestImmunizationData.filter(
    item => dropoutIndicators.includes(item.indicator)
  ).sort((a, b) => {
  ...
  const wastageData = latestImmunizationData.filter(
    item => wastageIndicators.includes(item.indicator)
  ).sort((a, b) => {
  ...
  const trendData = keyTrendIndicators.map(indicator => {
    const yearValues = Object.keys(multiYearData).map(year => {
      const data = multiYearData[year].find(item => item.indicator === indicator);
      return {
  ...
  const fullyImmunizedData = coverageData.find(item => item.indicator === "FULLY_IMMUNIZED_NIP_SCHEDULE");
  ...
  const dpt3Data = coverageData.find(item => item.indicator === "DPT_HEPB_HIB3_COVERAGE");
  ...
  const validKeyIndicatorValues = keyIndicators
    .map(ind => coverageData.find(item => item.indicator === ind)?.value || 0)
    .filter(val => val > 0);
  
  const immunizationQualityIndex = validKeyIndicatorValues.length > 0
    ? validKeyIndicatorValues.reduce((sum, val) => sum + val, 0) / validKeyIndicatorValues.length
    : 0;

  // Find DPT dropout rate
  const dptDropoutData = dropoutData.find(item => item.indicator === "DPT_HEPB_HIB_DROPOUT");
        ...
        <section>
          <div className="relative rounded-lg overflow-hidden mb-8">
            <Image
              ...
              width={1200}
              height={400}
              alt="खोप सेवा र कभरेज - खजुरा गाउँपालिका (Immunization Services and Coverage - Khajura Rural Municipality)"
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none">
            <h1 className="scroll-m-20 tracking-tight mb-6">
              खजुरा गाउँपालिकामा खोप सेवा र कभरेजको अवस्था
            </h1>
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none mt-8">
            <h2 id="immunization-service-improvement-strategy" className="scroll-m-20 border-b pb-2 mt-12">
              खोप सेवा सुधार रणनीति
            </h2>

            <p>
              खजुरा गाउँपालिकामा खोप सेवाको अवस्था विश्लेषणबाट निम्न रणनीतिहरू 
              अवलम्बन गर्न सकिन्छ:
            </p>
