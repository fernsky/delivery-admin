...
interface WardHealthInsuredHouseholdsPieChartsProps {
  wardWiseData: Array<Record<string, any>>;
}

export default function WardHealthInsuredHouseholdsPieCharts({
  wardWiseData,
}: WardHealthInsuredHouseholdsPieChartsProps) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {wardWiseData.map((wardData) => {
        ...
        const insuranceCategories = [
          {
            name: "बीमा गरेका",
            value: wardData["बीमा गरेका"] || 0,
            color: INSURANCE_CATEGORIES["बीमा गरेका"].color
          },
          {
            name: "बीमा नगरेका",
            value: wardData["बीमा नगरेका"] || 0,
            color: INSURANCE_CATEGORIES["बीमा नगरेका"].color
          }
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {insuranceCategories.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={entry.color}
                      />
