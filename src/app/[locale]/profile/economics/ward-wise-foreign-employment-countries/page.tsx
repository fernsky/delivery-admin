...
const COUNTRY_NAMES: Record<string, string> = foreignEmploymentCountryOptions.reduce(
  (acc, option) => ({
    ...acc,
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    ...
    const employmentData = await api.profile.economics.wardWiseForeignEmploymentCountries.getAll.query();
    ...
    const mostCommonPercentage = totalPopulation > 0
      ? ((mostCommonCount / totalPopulation) * 100).toFixed(2)
      : "0";

    // Create rich keywords with actual data
    const keywordsNP = [
      "खजुरा गाउँपालिका वैदेशिक रोजगारी",
      "खजुरा वैदेशिक रोजगारीका गन्तव्य देशहरू",
      "वडा अनुसार वैदेशिक रोजगारी",
      "विदेशमा रोजगारीमा गएका खजुरावासीहरू",
      COUNTRY_NAMES[mostCommonCountry] || mostCommonCountry,
      `खजुरा वैदेशिक रोजगारी संख्या ${localizeNumber(totalPopulation.toString(), "ne")}`,
    ...
    const descriptionNP = `खजुरा गाउँपालिकाको वडा अनुसार वैदेशिक रोजगारीमा गएका जनसंख्याको वितरण र विश्लेषण। कुल ${localizeNumber(totalPopulation.toString(), "ne")} जना वैदेशिक रोजगारीमा रहेका छन्, जसमध्ये ${localizeNumber(mostCommonPercentage, "ne")}% (${localizeNumber(mostCommonCount.toString(), "ne")}) ${COUNTRY_NAMES[mostCommonCountry] || mostCommonCountry} मा रहेका छन्। विभिन्न वडाहरूमा वैदेशिक रोजगारीको विस्तृत विश्लेषण।`;
    ...
    return {
      title: "वैदेशिक रोजगारीका गन्तव्य देशहरू | खजुरा गाउँपालिका डिजिटल प्रोफाइल",
      description: "वडा अनुसार वैदेशिक रोजगारीमा गएका जनसंख्याको वितरण र विश्लेषण।",
    };
  }
}
...
const toc = [
  ...
  { level: 2, text: "प्रमुख गन्तव्य देशहरू", slug: "main-destination-countries" },
  { level: 2, text: "वडा अनुसार वैदेशिक रोजगारी", slug: "ward-wise-foreign-employment" },
  { level: 2, text: "क्षेत्रगत वैदेशिक रोजगारी प्रवृत्ति", slug: "regional-foreign-employment-trends" },
  { level: 2, text: "आर्थिक प्रभाव र रेमिट्यान्स", slug: "economic-impact-and-remittance" },
  { level: 2, text: "सीप विकास र फिर्ती रणनीति", slug: "skill-development-and-return-strategy" },
...
export default async function WardWiseForeignEmploymentCountriesPage() {
  ...
  try {
    summaryData = await api.profile.economics.wardWiseForeignEmploymentCountries.summary.query();
  } catch (error) {
  ...
  const regionData = employmentData.reduce((acc: Record<string, number>, item) => {
  ...
  const wardWiseAnalysis = wardNumbers.map((wardNumber) => {
    ...
    const mostCommonCountry = wardData.reduce((prev, current) => {
    ...
    const gulfCountriesPopulation = wardData
      .filter(item => ["QATAR", "SAUDI_ARABIA", "UNITED_ARAB_EMIRATES", "KUWAIT", "BAHRAIN", "OMAN"].includes(item.country))
      .reduce((sum, item) => sum + (item.population || 0), 0);
      
    // Calculate population in Asia Pacific
    const asiaPacificPopulation = wardData
      .filter(item => ["MALAYSIA", "JAPAN", "SOUTH_KOREA", "SINGAPORE", "AUSTRALIA", "THAILAND"].includes(item.country))
      .reduce((sum, item) => sum + (item.population || 0), 0);
      
    // Calculate population in Western countries
    const westernCountriesPopulation = wardData
      .filter(item => ["UNITED_STATES_OF_AMERICA", "CANADA", "UNITED_KINGDOM_OF_GREAT_BRITAIN", "GERMANY", "ITALY"].includes(item.country))
      .reduce((sum, item) => sum + (item.population || 0), 0);
    
    const gulfPercentage = wardTotalPopulation > 0 
      ? ((gulfCountriesPopulation / wardTotalPopulation) * 100).toFixed(2)
      : "0";
      
    const asiaPacificPercentage = wardTotalPopulation > 0 
      ? ((asiaPacificPopulation / wardTotalPopulation) * 100).toFixed(2)
      : "0";
      
    const westernCountriesPercentage = wardTotalPopulation > 0 
      ? ((westernCountriesPopulation / wardTotalPopulation) * 100).toFixed(2)
      : "0";
    ...
    const countryDistribution = wardData.reduce((acc: Record<string, number>, item) => {
    ...
    return {
      ...
      mostCommonCountryPercentage: wardTotalPopulation > 0 
        ? ((mostCommonCountry.population || 0) / wardTotalPopulation * 100).toFixed(2)
        : "0",
  ...
  const estimatedAnnualRemittance = totalPopulation * averageAnnualRemittancePerPerson;
  const formattedEstimatedAnnualRemittance = (estimatedAnnualRemittance / 10000000).toFixed(2); // Convert to crores
  ...
  const skillLevelData = employmentData.reduce((acc: Record<string, number>, item) => {
        ...
        <section>
          <div className="relative rounded-lg overflow-hidden mb-8">
            <Image
              ...
              width={1200}
              height={400}
              alt="वैदेशिक रोजगारीका गन्तव्य देशहरू - खजुरा गाउँपालिका (Foreign Employment Destinations - Khajura Rural Municipality)"
          ...
          <div className="prose prose-slate dark:prose-invert max-w-none">
            ...
            <p>
              खजुरा गाउँपालिकाको वैदेशिक रोजगारी सम्बन्धी तथ्याङ्क अनुसार, कुल {localizeNumber(totalPopulation.toLocaleString(), "ne")} 
              जना विभिन्न देशहरूमा रोजगारीका लागि गएका छन्। यसमध्ये सबैभन्दा बढी {overallSummary[0]?.countryName || ""} 
              मा {localizeNumber(((overallSummary[0]?.population || 0) / totalPopulation * 100).toFixed(1), "ne")}% 
              अर्थात् {localizeNumber((overallSummary[0]?.population || 0).toLocaleString(), "ne")} जना रहेका छन्।
            </p>
            ...
            <p>
              खजुरा गाउँपालिकाका वासिन्दाहरूको वैदेशिक रोजगारीका प्रमुख गन्तव्य देशहरू र त्यहाँ रहेको जनसंख्या निम्नानुसार
              रहेको छ:
            </p>
            ...
            <ul>
              {overallSummary.slice(0, 5).map((item, index) => (
                <li key={index}>
                  <strong>{item.countryName}</strong>: कुल {localizeNumber(((item.population / totalPopulation) * 100).toFixed(1), "ne")}% 
                  ({localizeNumber(item.population.toLocaleString(), "ne")} जना)
                </li>
              ))}
              <li>
                <strong>अन्य देशहरू</strong>: कुल {localizeNumber(
                  ((overallSummary.slice(5).reduce((sum, item) => sum + item.population, 0) / totalPopulation) * 100).toFixed(1),
                  "ne"
                )}% ({localizeNumber(
                  overallSummary.slice(5).reduce((sum, item) => sum + item.population, 0).toLocaleString(),
                  "ne"
                )} जना)
              </li>
            </ul>
            ...
            <p>
              क्षेत्रगत विश्लेषण गर्दा, खजुरा गाउँपालिकाका {localizeNumber(
                ((regionData['खाडी मुलुक'] / totalPopulation * 100) || 0).toFixed(1),
                "ne"
              )}% वैदेशिक रोजगारीमा गएकाहरू खाडी मुलुकहरूमा छन्, जसमा {overallSummary[0]?.countryName || ""}, {overallSummary.find(s => s.country === "SAUDI_ARABIA")?.countryName || ""} र {overallSummary.find(s => s.country === "UNITED_ARAB_EMIRATES")?.countryName || ""} प्रमुख छन्।
            </p>

            <p>
              दक्ष र अर्धदक्ष कामदारहरूको सन्दर्भमा, लगभग {localizeNumber((parseInt(skillLevelChartData.find(s => s.name === "दक्ष")?.percentage || "0") + parseInt(skillLevelChartData.find(s => s.name === "अर्धदक्ष")?.percentage || "0")).toFixed(1), "ne")}% जनशक्ति दक्ष र अर्धदक्ष श्रमिकको रूपमा विदेशमा कार्यरत छन्।
            </p>
          </div>
          ...
          <ForeignEmploymentCountriesCharts
            ...
            estimatedAnnualRemittance={estimatedAnnualRemittance}
            formattedEstimatedAnnualRemittance={formattedEstimatedAnnualRemittance}
          />

          <div className="prose prose-slate dark:prose-invert max-w-none mt-8">
            <h2 id="economic-impact-and-remittance" className="scroll-m-20 border-b pb-2">
              आर्थिक प्रभाव र रेमिट्यान्स
            </h2>
            <p>
              खजुरा गाउँपालिकामा वैदेशिक रोजगारीबाट प्राप्त रेमिट्यान्सले स्थानीय अर्थतन्त्रमा महत्वपूर्ण योगदान पुर्‍याएको छ। 
              अनुमानित हिसाबले वार्षिक रु. {localizeNumber(formattedEstimatedAnnualRemittance, "ne")} करोड रेमिट्यान्स पालिकामा भित्रिने अनुमान गरिएको छ।
            </p>
            
            <p>
              यो रकम घरपरिवारको जीवनस्तर उकास्न, शिक्षा र स्वास्थ्यमा लगानी गर्न, घरजग्गा खरिद गर्न तथा साना व्यवसाय सञ्चालन गर्न उपयोग भइरहेको छ। 
              तर रेमिट्यान्सको उत्पादनशील क्षेत्रमा पर्याप्त लगानी भने हुन नसकेको अवस्था पनि छ।
            </p>
            ...
            <h2 id="skill-development-and-return-strategy" className="scroll-m-20 border-b pb-2 mt-12">
              सीप विकास र फिर्ती रणनीति
            </h2>
            
            <p>
              वैदेशिक रोजगारीबाट फर्किएका व्यक्तिहरूमा सीप, अनुभव र पुँजी हुने हुनाले स्थानीय विकासका लागि यो महत्वपूर्ण स्रोत हो।
              खजुरा गाउँपालिकाले वैदेशिक रोजगारीमा जाने व्यक्तिहरूलाई गन्तव्य देश अनुसार सीपमूलक तालिम प्रदान गर्ने र फर्केर आउनेहरूलाई 
              उद्यमशीलता विकासमा सहयोग गर्ने नीति लिएको छ।
            </p>
            ...
            <div className="pl-6 space-y-4">
              <div className="flex">
                ...
                <div>
                  <strong>गन्तव्य विविधिकरण:</strong> हाल {localizeNumber(
                    ((regionData['खाडी मुलुक'] / totalPopulation * 100) || 0).toFixed(0),
                    "ne"
                  )}% भन्दा बढी वैदेशिक रोजगारी खाडी मुलुकहरूमा केन्द्रित भएकाले अधिक सुरक्षित र उच्च आम्दानी हुने मुलुकहरूमा
                  रोजगारी प्रवद्र्धन गर्नुपर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">२.</span>
                <div>
                  <strong>सीपमूलक तालिम:</strong> अदक्ष कामदारको रूपमा जाने संख्या {localizeNumber(
                    skillLevelChartData.find(s => s.name === "अदक्ष")?.percentage || "0", 
                    "ne"
                  )}% रहेकोले दक्ष र अर्धदक्ष जनशक्ति उत्पादनका लागि लक्षित तालिम कार्यक्रम सञ्चालन गर्नुपर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">३.</span>
                <div>
                  <strong>वडागत लक्षित कार्यक्रम:</strong> {wardWiseAnalysis.slice(0, 3).map((ward, i, arr) => (
                    i === arr.length - 1 
                      ? `र वडा नं. ${localizeNumber(ward.wardNumber.toString(), "ne")}` 
                      : `वडा नं. ${localizeNumber(ward.wardNumber.toString(), "ne")}, `
                  ))}
                  मा वैदेशिक रोजगारीमा जानेको संख्या बढी भएकाले यी वडाहरूमा सुरक्षित वैदेशिक रोजगारी सम्बन्धी विशेष सूचना केन्द्रहरू स्थापना गर्नुपर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">४.</span>
                <div>
                  <strong>उत्पादनशील क्षेत्रमा रेमिट्यान्स परिचालन:</strong> अनुमानित वार्षिक रु. {localizeNumber(formattedEstimatedAnnualRemittance, "ne")} करोड रेमिट्यान्सलाई उत्पादनशील क्षेत्रमा लगानी गर्न प्रोत्साहित गर्ने नीति र कार्यक्रम ल्याउनुपर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">५.</span>
                <div>
                  <strong>स्थानीय रोजगारी सिर्जना:</strong> दीर्घकालीन रूपमा वैदेशिक रोजगारीमाथिको निर्भरता घटाउन स्थानीय रोजगारी सिर्जना हुने क्षेत्रहरूमा लगानी बढाउनुपर्ने।
                </div>
              </div>
              <div className="flex">
                <span className="font-bold mr-2">६.</span>
                <div>
                  <strong>फिर्ती योजना:</strong> वैदेशिक रोजगारीबाट फिर्ता आएकाहरूका लागि सीप रूपान्तरण र उद्यमशीलता विकास कार्यक्रम सञ्चालन गर्नुपर्ने।
                </div>
              </div>
            </div>
