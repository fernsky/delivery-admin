...
export default function HouseholdOuterWallAnalysisSection({
  ...
  const municipalityQualityScore = wardWiseAnalysis.reduce((sum, ward) => {
    return sum + (ward.qualityScore * ward.totalHouseholds);
  }, 0) / totalHouseholds;

  // Calculate cement-bonded wall percentage for the municipality
  const cementJoinedData = overallSummary.find(item => item.wallType === "CEMENT_JOINED");
  ...
  const highestQualityWard = [...wardWiseAnalysis].sort(
    (a, b) => parseFloat(b.highQualityPercentage) - parseFloat(a.highQualityPercentage)
  )[0];
  ...
  const mudJoinedData = overallSummary.find(item => item.wallType === "MUD_JOINED");
  ...
  const vulnerableMaterialData = overallSummary.filter(
    item => ["BAMBOO", "TIN", "OTHER"].includes(item.wallType)
  );
  const vulnerableMaterialPercentage = vulnerableMaterialData.length
    ? ((vulnerableMaterialData.reduce((sum, item) => sum + item.households, 0) / totalHouseholds) * 100).toFixed(2)
    : "0";
  ...
  const priorityWard = [...wardWiseAnalysis].sort(
    (a, b) => parseFloat(b.lowQualityPercentage) - parseFloat(a.lowQualityPercentage)
  )[0];
  ...
  const seoAttributes = {
    ...
    "data-most-common-wall": overallSummary.length > 0 ? 
    ...
    "data-high-quality-percentage": wardWiseAnalysis.length > 0 ?
      ((wardWiseAnalysis.reduce((sum, ward) => sum + ward.highQualityWalls, 0) / totalHouseholds) * 100).toFixed(2) : "0",
  };
    ...
    <>
      ...
      >
        ...
        <div className="bg-muted/50 rounded-lg p-4 text-center min-w-[200px] border">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            ({localizeNumber((cementJoinedData?.households || 0).toLocaleString(), "ne")} घरधुरी)
          </p>
        </div>
      ...
      <div className="bg-muted/50 p-6 rounded-lg mt-8 border">
        <h3 className="text-xl font-medium mb-6">
          ...
          <span className="sr-only">Detailed House Outer Wall Analysis of Khajura</span>
        </h3>
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <div
                ...
                style={{
                  backgroundColor: overallSummary.length > 0 
                    ? WALL_TYPE_COLORS[overallSummary[0].wallType as keyof typeof WALL_TYPE_COLORS] || "#888"
                    : "#888"
                }}
              ></div>
              <div>
                <p className="text-2xl font-bold">
                  {overallSummary.length > 0 ? overallSummary[0].wallTypeName.split('(')[0].trim() : ""}
                </p>
                <p className="text-sm text-muted-foreground mt-1">
                  {localizeNumber(overallSummary.length > 0 ? ((overallSummary[0].households / totalHouseholds) * 100).toFixed(2) : "0", "ne")}% 
                  ({localizeNumber(overallSummary.length > 0 ? overallSummary[0].households.toLocaleString() : "0", "ne")} घरधुरी)
                </p>
              </div>
            </div>
              ...
              {overallSummary.slice(0, 3).map((item, index) => (
                <div key={index} className="mt-3">
                  <div className="flex justify-between text-sm">
                    <span>{index + 1}. {item.wallTypeName.split('(')[0].trim()}</span>
                    <span className="font-medium">{localizeNumber(((item.households / totalHouseholds) * 100).toFixed(1), "ne")}%</span>
                  </div>
                  <div className="w-full bg-muted h-2 rounded-full mt-1 overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        backgroundColor: WALL_TYPE_COLORS[item.wallType as keyof typeof WALL_TYPE_COLORS] || "#888",
                      }}
                    ></div>
                  </div>
                </div>
          ...
          <div className="bg-card p-4 rounded border">
            ...
            <div className="space-y-3">
              {highQualityDistribution.slice(0, 5).map((ward, index) => (
                <div key={index}>
                  <div className="flex justify-between text-sm">
                    <span>वडा {localizeNumber(ward.wardNumber.toString(), "ne")} - उच्च गुणस्तर</span>
                    <span className="font-medium">{localizeNumber(ward.percentage.toFixed(1), "ne")}%</span>
                  </div>
                  <div className="w-full bg-muted h-2 rounded-full mt-1 overflow-hidden">
                    <div
            ...
            <div className="mt-6 pt-4 border-t">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">उच्च गुणस्तरका गारो भएका घरहरू</p>
                </div>
                <div className="text-right">
                  <p className="text-xl font-bold">वडा {localizeNumber(highestQualityWard?.wardNumber.toString() || "", "ne")}</p>
                  <p className="text-sm text-blue-500 font-medium">
                    {localizeNumber(highestQualityWard?.highQualityPercentage || "0", "ne")}%
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-card p-4 rounded border">
            ...
            <ul className="space-y-2 text-sm">
              <li className="flex gap-2">
                ...
                <span>
                  <strong>प्राथमिकता वडा:</strong> वडा नं. {localizeNumber(priorityWard?.wardNumber.toString() || "", "ne")} 
                  मा न्यून गुणस्तरका गारो भएका घरहरू {localizeNumber(priorityWard?.lowQualityPercentage || "0", "ne")}% रहेकाले 
                  त्यहाँको आवासीय सुरक्षा सुधारलाई प्राथमिकता दिनुपर्ने।
                </span>
              </li>
              {parseFloat(vulnerableMaterialPercentage) > 10 && (
                <li className="flex gap-2">
                  ...
                  <span>
                    <strong>जोखिम न्यूनीकरण:</strong> जस्ता/टिन, बाँसजन्य र अन्य कमजोर सामग्री प्रयोग गरिएका 
                    {localizeNumber(vulnerableMaterialPercentage, "ne")}% घरहरू प्राकृतिक विपद्को उच्च जोखिममा रहेकाले 
                    त्यस्ता घरहरूको सुधारका लागि विशेष कार्यक्रम ल्याउनुपर्ने।
                  </span>
                </li>
              )}
              {parseFloat(cementJoinedPercentage) < 40 && (
                <li className="flex gap-2">
                  ...
                  <span>
                    <strong>भवन मापदण्ड:</strong> पालिकामा सिमेन्टको जोडाइ भएको गारोका घरहरू जम्मा 
                    {localizeNumber(cementJoinedPercentage, "ne")}% मात्र रहेकाले भूकम्प प्रतिरोधी निर्माणका लागि 
                    भवन मापदण्ड कार्यान्वयनमा जोड दिनुपर्ने।
                  </span>
                </li>
              )}
              <li className="flex gap-2">
                ...
                <span>
                  <strong>प्रविधि हस्तान्तरण:</strong> स्थानीय निर्माण व्यवसायी र कामदारहरूलाई गुणस्तरीय 
                  गारो निर्माण तालिमको व्यवस्था गरी प्रविधि हस्तान्तरण गर्नुपर्ने।
                </span>
              </li>
              <li className="flex gap-2">
                <span className="text-red-500">•</span>
                <span>
                  <strong>सचेतनामूलक कार्यक्रम:</strong> गारोको गुणस्तर र भूकम्पीय सुरक्षासम्बन्धी जनचेतना 
                  अभिवृद्धि कार्यक्रमहरू संचालन गर्नुपर्ने।
                </span>
              </li>
            </ul>
          </div>
          ...
          <div className="bg-card p-4 rounded border">
            ...
            <div className="space-y-5">
              <div>
                <h5 className="text-sm font-medium mb-1">उच्च गुणस्तरका घरहरू</h5>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">
                    {localizeNumber(
                      (wardWiseAnalysis.reduce((sum, ward) => sum + parseFloat(ward.highQualityPercentage), 0) / 
                      wardWiseAnalysis.length).toFixed(1), 
                      "ne"
                    )}%
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">सिमेन्टको जोडाइ भएको इँटा/ढुङ्गा र प्रि फ्याब</p>
              </div>
              
              <div>
                <h5 className="text-sm font-medium mb-1">मध्यम गुणस्तरका घरहरू</h5>
                <div className="flex items-center gap-2">
                  <div className="w-full bg-muted h-4 rounded-full overflow-hidden">
                    <div 
                  ...
                  <span className="text-sm font-medium">
                    {localizeNumber(
                      (wardWiseAnalysis.reduce((sum, ward) => sum + parseFloat(ward.mediumQualityPercentage), 0) / 
                      wardWiseAnalysis.length).toFixed(1), 
                      "ne"
                    )}%
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">माटोको जोडाइ भएको र काठ/फल्याकको गारो</p>
              </div>
              
              <div>
                <h5 className="text-sm font-medium mb-1">न्यून गुणस्तरका घरहरू</h5>
                <div className="flex items-center gap-2">
                  <div className="w-full bg-muted h-4 rounded-full overflow-hidden">
                    <div 
                  ...
                  <span className="text-sm font-medium">
                    {localizeNumber(
                      (wardWiseAnalysis.reduce((sum, ward) => sum + parseFloat(ward.lowQualityPercentage), 0) / 
                      wardWiseAnalysis.length).toFixed(1), 
                      "ne"
                    )}%
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">काँचो इँटा, जस्ता/टिन, बाँसजन्य सामग्री</p>
              </div>
            </div>
          </div>
        </div>
