...
type WardAgeGenderWiseAbsenteeData = {
  id: string;
  wardId: string;
  wardNumber?: number;
  ageGroup: string;
...
interface WardAgeGenderWiseAbsenteeTableProps {
...
export default function WardAgeGenderWiseAbsenteeTable({
  data,
  onEdit,
}: WardAgeGenderWiseAbsenteeTableProps) {
  ...
  const deleteWardAgeGenderWiseAbsentee =
    api.profile.demographics.wardAgeGenderWiseAbsentee.delete.useMutation({
      onSuccess: () => {
  ...
  const handleDelete = () => {
    if (deleteId) {
      deleteWardAgeGenderWiseAbsentee.mutate({ id: deleteId });
      setDeleteId(null);
    }
  };
  ...
  const groupedByWard = filteredData.reduce(
    ...
    {} as Record<
      string,
      {
        wardId: string;
        wardNumber: number;
        items: WardAgeGenderWiseAbsenteeData[];
      }
    >,
  );
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-sm font-normal text-muted-foreground hidden md:block">
                          <span className="mr-4">
                            प्रवासी जनसंख्या: {totalPopulation.toLocaleString()}
                          </span>
                        </div>
                        {isExpanded ? (
                ...
                <Table className="min-w-max">
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">कुल प्रवासी</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {uniqueGenders.map((gender) => {
                      ...
                      return sortedWardGroups
                        .map((wardGroup) => {
                          ...
                          const ageGroupMap = genderData.reduce(
                            ...
                            {} as Record<string, WardAgeGenderWiseAbsenteeData>,
                          );
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              वडा अनुसार उमेर र लिङ्ग प्रवासी डाटा मेट्ने?
            </AlertDialogTitle>
            <AlertDialogDescription>
