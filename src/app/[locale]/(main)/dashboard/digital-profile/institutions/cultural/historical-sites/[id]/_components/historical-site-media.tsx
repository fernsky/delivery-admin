...
interface HistoricalSiteMediaProps {
  historicalSiteId: string;
  media: MediaFile[];
}

export function HistoricalSiteMedia({
  historicalSiteId,
  media,
}: HistoricalSiteMediaProps) {
  ...
  if (!media || media.length === 0) {
    return (
      <Card className="flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
        ...
        <p>यस ऐतिहासिक स्थलको कुनै फोटोहरू अपलोड गरिएको छैन।</p>
      </Card>
    );
  }
        ...
        {media.map((item) => (
          ...
          >
            <div className="relative h-64 w-full">
              <Image
                src={item.url}
                alt={item.title || "Historical site image"}
                fill
          ...
          <div className="relative h-[60vh] w-full">
            {selectedImage && (
              <Image
                ...
                alt={selectedImage.title || "Historical site image"}
                fill
