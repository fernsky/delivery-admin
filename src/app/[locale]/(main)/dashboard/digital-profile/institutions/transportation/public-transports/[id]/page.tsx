"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { api } from "@/trpc/react";
import { ContentLayout } from "@/components/admin-panel/content-layout";
import { Button } from "@/components/ui/button";
import { Loader, ChevronLeft, Edit, Trash2 } from "lucide-react";
import { PublicTransportDetails } from "./_components/public-transport-details";
import { PublicTransportMedia } from "./_components/public-transport-media";
import { PublicTransportMap } from "./_components/public-transport-map";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent,
  AlertDialogDescription,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
} from "@/components/ui/alert-dialog";
import { toast } from "sonner";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

export default function ViewPublicTransportPage({
  params,
}: {
  params: { id: string };
}) {
  const router = useRouter();
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);

  // Fetch public transport data by ID
  const { data: transport, isLoading } =
    api.profile.transportation.publicTransports.getById.useQuery(params.id, {
      retry: false,
      enabled: !!params.id,
      onError: () => {
        router.push(
          "/dashboard/digital-profile/institutions/transportation/public-transports",
        );
        toast.error("सार्वजनिक यातायात फेला परेन");
      },
    });

  // Delete mutation
  const { mutate: deletePublicTransport, isLoading: isDeleting } =
    api.profile.transportation.publicTransports.delete.useMutation({
      onSuccess: () => {
        toast.success("सार्वजनिक यातायात सफलतापूर्वक हटाइयो");
        router.push(
          "/dashboard/digital-profile/institutions/transportation/public-transports",
        );
      },
      onError: (error) => {
        toast.error(`सार्वजनिक यातायात हटाउन असफल: ${error.message}`);
        setIsDeleteDialogOpen(false);
      },
    });

  // Handle delete confirmation
  const handleDelete = () => {
    if (transport?.id) {
      deletePublicTransport(transport.id);
    }
  };

  // Get transport type label
  const getTransportTypeLabel = (type: string) => {
    const types = {
      BUS: "बस",
      MINIBUS: "मिनी बस",
      MICROBUS: "माइक्रो बस",
      TEMPO: "टेम्पो",
      AUTO_RICKSHAW: "अटो रिक्सा",
      TAXI: "ट्याक्सी",
      E_RICKSHAW: "इ-रिक्सा",
      OTHER: "अन्य",
    };
    return types[type as keyof typeof types] || type;
  };

  if (isLoading) {
    return (
      <ContentLayout title="सार्वजनिक यातायात लोड गर्दै...">
        <div className="flex justify-center items-center h-64">
          <Loader className="h-8 w-8 animate-spin text-primary" />
        </div>
      </ContentLayout>
    );
  }

  if (!transport) {
    return (
      <ContentLayout title="सार्वजनिक यातायात फेला परेन">
        <div className="flex flex-col items-center justify-center h-64 gap-4">
          <p className="text-muted-foreground">
            माग गरिएको सार्वजनिक यातायात फेला परेन
          </p>
          <Button
            onClick={() =>
              router.push(
                "/dashboard/digital-profile/institutions/transportation/public-transports",
              )
            }
            variant="outline"
          >
            <ChevronLeft className="mr-2 h-4 w-4" />
            सूचीमा फर्कनुहोस्
          </Button>
        </div>
      </ContentLayout>
    );
  }

  return (
    <ContentLayout
      title={transport.name}
      subtitle={getTransportTypeLabel(transport.type)}
      backHref="/dashboard/digital-profile/institutions/transportation/public-transports"
      actions={
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() =>
              router.push(
                `/digital-profile/institutions/transportation/public-transports/edit/${transport.id}`,
              )
            }
          >
            <Edit className="mr-2 h-4 w-4" />
            सम्पादन
          </Button>
          <Button
            variant="destructive"
            onClick={() => setIsDeleteDialogOpen(true)}
          >
            <Trash2 className="mr-2 h-4 w-4" />
            हटाउनुहोस्
          </Button>
        </div>
      }
    >
      <div className="space-y-8">
        <Tabs defaultValue="details">
          <TabsList className="mb-6">
            <TabsTrigger value="details">विवरण</TabsTrigger>
            <TabsTrigger value="map">नक्सा</TabsTrigger>
            <TabsTrigger value="media">फोटोहरू</TabsTrigger>
          </TabsList>

          <TabsContent value="details">
            <PublicTransportDetails transport={transport} />
          </TabsContent>

          <TabsContent value="map">
            <PublicTransportMap transport={transport} />
          </TabsContent>

          <TabsContent value="media">
            <PublicTransportMedia
              transportId={transport.id}
              media={transport.media || []}
            />
          </TabsContent>
        </Tabs>
      </div>

      {/* Delete confirmation dialog */}
      <AlertDialog
        open={isDeleteDialogOpen}
        onOpenChange={setIsDeleteDialogOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>के तपाईं निश्चित हुनुहुन्छ?</AlertDialogTitle>
            <AlertDialogDescription>
              सार्वजनिक यातायात &quot;{transport.name}&quot; हटाउने। यो कार्य
              पूर्ववत हुन सक्दैन।
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>रद्द गर्नुहोस्</AlertDialogCancel>
            <AlertDialogAction onClick={handleDelete} disabled={isDeleting}>
              {isDeleting && <Loader className="mr-2 h-4 w-4 animate-spin" />}
              हटाउनुहोस्
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </ContentLayout>
  );
}
