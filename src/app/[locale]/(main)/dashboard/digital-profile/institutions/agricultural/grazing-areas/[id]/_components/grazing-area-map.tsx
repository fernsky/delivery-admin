...
interface GrazingAreaMapProps {
  grazingArea: any;
}

export function GrazingAreaMap({ grazingArea }: GrazingAreaMapProps) {
  ...
  useEffect(() => {
    ...
    const pointStyle = new Style({
      image: new Circle({
        radius: 8,
        fill: new Fill({ color: "#8b5cf6" }), // Purple for grazing areas
    ...
    const polygonStyle = new Style({
      ...
      fill: new Fill({
        color: "rgba(139, 92, 246, 0.2)", // Semi-transparent purple
      }),
    });
    ...
    if (grazingArea.areaPolygon) {
      try {
        // Convert the coordinates from [long, lat] to OpenLayers format
        const coords = grazingArea.areaPolygon.coordinates[0].map(
    ...
    if (grazingArea.locationPoint) {
      try {
        const point = new OLPoint(
          fromLonLat(grazingArea.locationPoint.coordinates),
        );
        ...
        if (!grazingArea.areaPolygon) {
          mapRef.current
            .getView()
            .setCenter(fromLonLat(grazingArea.locationPoint.coordinates));
    ...
    if (!grazingArea.locationPoint && !grazingArea.areaPolygon) {
      ...
      <CardContent className="p-0">
        <div className="relative h-[600px]">
          ...
          <div className="absolute bottom-4 right-4 z-10 bg-black/50 text-white p-2 rounded max-w-xs text-xs">
            <p className="font-medium mb-1">{grazingArea.name}</p>
            <p>
              {grazingArea.location ? `${grazingArea.location}, ` : ""}
              {grazingArea.wardNumber ? `वडा नं ${grazingArea.wardNumber}` : ""}
            </p>
            {grazingArea.areaInHectares && (
              <p className="mt-1">
                {grazingArea.areaInHectares} हेक्टर क्षेत्रफल
              </p>
            )}
          </div>
        </div>
