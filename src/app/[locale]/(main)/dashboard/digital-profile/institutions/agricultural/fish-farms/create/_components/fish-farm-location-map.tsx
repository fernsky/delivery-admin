...
interface FishFarmLocationMapProps {
  formData: FormData;
  updateMapData: (data: {
...
export default function FishFarmLocationMap({
  formData,
  updateMapData,
  updateFormData,
}: FishFarmLocationMapProps) {
  ...
  // Handle polygon drawing completion (farm boundary)
  ...
  // Handle multiple polygon drawing (for ponds)
  const handleMultiPolygonComplete = (
    coordinates: Array<Array<Array<[number, number]>>>,
  ) => {
    updateMapData({
      pondPolygons: {
      ...
      <div>
        ...
        <Card className="mt-1 overflow-hidden">
          <MapComponent
            ...
            onMultiPolygonComplete={handleMultiPolygonComplete}
            initialMarker={
            ...
            initialMultiPolygons={
              formData.pondPolygons
                ? formData.pondPolygons.coordinates.map((poly) =>
                    poly[0].map((coord) => ({
                      lat: coord[1],
                      lng: coord[0],
                    })),
