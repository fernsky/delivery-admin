...
import { GrasslandLocationMap } from "../../create/_components/grassland-location-map";
import { BasicGrasslandDetails } from "../../create/_components/basic-grassland-details";
import { GrasslandFeaturesDetails } from "../../create/_components/grassland-features-details";
...
interface GrasslandEditFormProps {
  grassland: any;
}

export function GrasslandEditForm({ grassland }: GrasslandEditFormProps) {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState("basic");

  // Set up the form with grassland data
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      // Location details
      wardNumber: grassland.wardNumber?.toString() || "",
      location: grassland.location || "",
      address: grassland.address || "",

      // Physical details
      areaInHectares: grassland.areaInHectares?.toString() || "",
      elevationInMeters: grassland.elevationInMeters?.toString() || "",
      vegetationDensity: grassland.vegetationDensity || undefined,
      managementType: grassland.managementType || undefined,

      // Grassland specific details
      dominantSpecies: grassland.dominantSpecies || "",
      carryingCapacity: grassland.carryingCapacity?.toString() || "",
      grazingPeriod: grassland.grazingPeriod || "",
      annualFodderYield: grassland.annualFodderYield?.toString() || "",
      yieldYear: grassland.yieldYear || "",

      // Management details
      isGovernmentOwned: grassland.isGovernmentOwned || false,
      ...
      // Additional information
      hasWaterSource: grassland.hasWaterSource || false,
      waterSourceType: grassland.waterSourceType || "",
      isFenced: grassland.isFenced || false,
      hasGrazingRights: grassland.hasGrazingRights || false,

      // Conservation status
      hasProtectedStatus: grassland.hasProtectedStatus || false,
      ...
      // Geometry fields
      locationPoint: grassland.locationPoint,
      areaPolygon: grassland.areaPolygon,
    },
  });

  // Update grassland mutation
  const { mutate: updateGrassland, isLoading } =
    api.profile.grasslands.update.useMutation({
      onSuccess: () => {
        toast.success("चरन क्षेत्र सफलतापूर्वक अपडेट गरियो");
        router.push(
          `/dashboard/digital-profile/institutions/agricultural/grasslands/${grassland.id}`,
        );
      },
      onError: (error) => {
        toast.error(`चरन क्षेत्र अपडेट गर्न असफल: ${error.message}`);
      },
    });
