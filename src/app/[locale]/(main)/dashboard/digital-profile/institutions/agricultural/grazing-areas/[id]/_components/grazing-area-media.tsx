...
import { Image, X, FileText, Video, FileImage, Sprout } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { api } from "@/trpc/react";
import { useRouter } from "next/navigation";
import { toast } from "sonner";

interface GrazingAreaMediaProps {
  grazingAreaId: string;
  media: any[];
}

export function GrazingAreaMedia({
  grazingAreaId,
  media,
}: GrazingAreaMediaProps) {
  ...
  const handleSetPrimary = (mediaId: string) => {
    setPrimaryMedia({
      mediaId,
      entityId: grazingAreaId,
      entityType: "GRAZING_AREA",
    });
  };
          ...
          {media.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm mt-2">
                चरन खर्क क्षेत्रको फोटोहरू संलग्न गर्न सम्पादन पृष्ठमा जानुहोस्
              </p>
            </div>
          ) : (
