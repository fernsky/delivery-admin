...
interface GrasslandMapProps {
  grassland: any;
}

export function GrasslandMap({ grassland }: GrasslandMapProps) {
  ...
  useEffect(() => {
    ...
    const pointStyle = new Style({
      image: new Circle({
        radius: 8,
        fill: new Fill({ color: "#10b981" }), // Green for grassland
    ...
    if (grassland.areaPolygon) {
      try {
        ...
        const coords = grassland.areaPolygon.coordinates[0].map(
    ...
    if (grassland.locationPoint) {
      try {
        const point = new OLPoint(
          fromLonLat(grassland.locationPoint.coordinates),
        );
        ...
        if (!grassland.areaPolygon) {
          mapRef.current
            .getView()
            .setCenter(fromLonLat(grassland.locationPoint.coordinates));
    ...
    if (!grassland.locationPoint && !grassland.areaPolygon) {
        ...
        <div className="relative h-[600px]">
          ...
          <div className="absolute bottom-4 left-4 z-10 bg-white px-3 py-2 rounded-md shadow-md text-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>चरन क्षेत्र सीमाना</span>
            </div>
          </div>

          {/* Location info */}
          <div className="absolute bottom-4 right-4 z-10 bg-black/50 text-white p-2 rounded max-w-xs text-xs">
            <p className="font-medium mb-1">{grassland.name}</p>
            <p>
              {grassland.location ? `${grassland.location}, ` : ""}
              {grassland.wardNumber ? `वडा नं ${grassland.wardNumber}` : ""}
            </p>
            {grassland.areaInHectares && (
              <p className="mt-1">
                {grassland.areaInHectares} हेक्टर क्षेत्रफल
              </p>
            )}
          </div>
        </div>
