...
import { Image, X, FileText, Video, FileImage, GrassIcon } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { api } from "@/trpc/react";
import { useRouter } from "next/navigation";
import { toast } from "sonner";

interface GrasslandMediaProps {
  grasslandId: string;
  media: any[];
}

export function GrasslandMedia({ grasslandId, media }: GrasslandMediaProps) {
  ...
  const handleSetPrimary = (mediaId: string) => {
    setPrimaryMedia({
      mediaId,
      entityId: grasslandId,
      entityType: "GRASSLAND",
    });
  };
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            चरन क्षेत्रको फोटोहरू
          </CardTitle>
        </CardHeader>
        <CardContent>
          {media.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              <GrassIcon className="h-20 w-20 mx-auto opacity-10 mb-4" />
              <p>कुनै फोटो फेला परेन</p>
              <p className="text-sm mt-2">
                चरन क्षेत्रको फोटोहरू संलग्न गर्न सम्पादन पृष्ठमा जानुहोस्
              </p>
            </div>
          ) : (
