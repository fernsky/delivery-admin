...
interface WardWiseHouseholdsLoanUseTableProps {
...
export default function WardWiseHouseholdsLoanUseTable({
  data,
  onEdit,
}: WardWiseHouseholdsLoanUseTableProps) {
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [filterWard, setFilterWard] = useState<string>("all");
  const [filterLoanUse, setFilterLoanUse] = useState<string>("all");
  ...
  const deleteWardWiseHouseholdsLoanUse =
    api.profile.economics.wardWiseHouseholdsLoanUse.delete.useMutation({
      onSuccess: () => {
  ...
  const uniqueLoanUses = Array.from(
    new Set(data.map((item) => item.loanUse)),
  ).sort();
      ...
      <div className="space-y-6">
        ...
        <div className="bg-muted/40 p-4 rounded-lg">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex flex-col space-y-2 flex-grow">
              <label
                htmlFor="loan-use-filter"
                className="text-sm font-medium text-muted-foreground"
              >
                ऋणको उपयोग अनुसार फिल्टर:
              </label>
              <Select value={filterLoanUse} onValueChange={setFilterLoanUse}>
                <SelectTrigger className="w-full sm:w-[240px]">
                  <SelectValue placeholder="सबै ऋणको उपयोग" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">सबै ऋणको उपयोग</SelectItem>
                  {uniqueLoanUses.map((loanUse) => (
                    <SelectItem key={loanUse} value={loanUse}>
                      {getLoanUseDisplayName(loanUse)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
        ...
        {sortedWardGroups.length === 0 ? (
          ...
          <div className="space-y-4">
            {viewMode === "list" ? (
                            ...
                            {wardGroup.items.map((item) => (
                              ...
                              >
                                <TableCell className="max-w-[240px] truncate">
                                  {getLoanUseDisplayName(item.loanUse)}
                                </TableCell>
              ...
              // Grid view - Loan uses as columns, wards as rows
              <div className="overflow-x-auto">
                <Table className="min-w-max">
                  <TableHeader>
                    <TableRow>
                      ...
                      {/* Generate column headers for each loan use */}
                      {uniqueLoanUses.map((loanUse) => (
                        <TableHead
                          key={loanUse}
                  ...
                  <TableBody>
                    {sortedWardGroups.map((wardGroup) => {
                      // For each ward, create a row with loan use data
                      const totalWardHouseholds = wardGroup.items.reduce(
                        (sum, item) => sum + (item.households || 0),
                        0,
                      );

                      // Create a mapping of loan use to item for this ward
                      const loanUseMap = wardGroup.items.reduce(
                        (map, item) => {
                          map[item.loanUse] = item;
                          return map;
                        },
                        {} as Record<string, WardWiseHouseholdsLoanUseData>,
                      );
                      ...
                      return (
                        <TableRow key={`grid-${wardGroup.wardId}`}>
                          ...
                          {/* Render cells for each loan use */}
                          {uniqueLoanUses.map((loanUse) => {
                    ...
                    {/* Summary row with totals by loan use */}
                    <TableRow className="bg-muted/20 font-medium">
                      <TableCell className="sticky left-0 bg-muted/20 z-10">
                        कुल जम्मा
                      </TableCell>
                      {uniqueLoanUses.map((loanUse) => {
                        const loanUseTotal = filteredData
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              वडा अनुसार ऋणको उपयोग डाटा मेट्ने?
            </AlertDialogTitle>
            <AlertDialogDescription>
