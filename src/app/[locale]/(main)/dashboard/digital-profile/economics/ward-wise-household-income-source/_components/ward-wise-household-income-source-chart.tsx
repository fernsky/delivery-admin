...
interface WardWiseHouseholdIncomeSourceChartProps {
...
export default function WardWiseHouseholdIncomeSourceChart({
  data,
}: WardWiseHouseholdIncomeSourceChartProps) {
  const [selectedWard, setSelectedWard] = useState<string>("all");
  const [chartView, setChartView] = useState<string>("byWard"); // 'byWard' or 'byIncomeSource'
  ...
  const uniqueIncomeSources = useMemo(() => {
    return Array.from(new Set(data.map((item) => item.incomeSource))).sort();
  }, [data]);
  ...
  const barChartData = useMemo(() => {
    if (chartView === "byWard") {
      ...
      return uniqueWards
        ...
        .map((wardId) => {
          ...
          uniqueIncomeSources.forEach((incomeSource) => {
            const incomeSourceItem = wardItems.find(
              (item) => item.incomeSource === incomeSource,
            );
            dataPoint[getIncomeSourceDisplayName(incomeSource)] =
              incomeSourceItem?.households || 0;
          });
      ...
      return uniqueIncomeSources.map((incomeSource) => {
        const incomeSourceItems = filteredData.filter(
          (item) => item.incomeSource === incomeSource,
        );

        // Create an object with income source as key
        const dataPoint: Record<string, any> = {
          incomeSource: getIncomeSourceDisplayName(incomeSource),
        };
    ...
    if (chartView === "byWard" && selectedWard !== "all") {
      ...
      return uniqueIncomeSources
        .map((incomeSource, index) => {
          ...
          return {
            id: getIncomeSourceDisplayName(incomeSource),
            label: getIncomeSourceDisplayName(incomeSource),
      ...
      // Create pie data showing total households by income source across all wards
      return uniqueIncomeSources
        .map((incomeSource, index) => {
  ...
  }, [filteredData, uniqueIncomeSources, chartView, selectedWard]);

  if (data.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500 text-lg">
          कुनै डाटा उपलब्ध छैन। पहिले वडा अनुसार घरधुरी आय स्रोत डाटा थप्नुहोस्।
        </p>
      </div>
    );
  }
  ...
  return (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle>वडा अनुसार घरधुरी आय स्रोत विश्लेषण</CardTitle>
        <CardDescription>
          वडा र आय स्रोत अनुसार घरधुरी डाटा हेर्नुहोस्
        </CardDescription>
          ...
          <div>
            ...
            <Select value={chartView} onValueChange={setChartView}>
              ...
              <SelectContent>
                ...
                <SelectItem value="byIncomeSource">आय स्रोत अनुसार</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <TabsContent value="bar">
            <div className="h-96 border rounded-lg p-4 bg-white">
              <ResponsiveBar
                data={barChartData}
                keys={chartKeys}
                indexBy={chartView === "byWard" ? "ward" : "incomeSource"}
                ...
                axisBottom={{
                  ...
                  tickPadding: 5,
                  tickRotation: 45,
                  legend: chartView === "byWard" ? "वडा" : "आय स्रोत",
          ...
          <Card className="p-4">
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              {uniqueIncomeSources.map((incomeSource) => {
                const incomeSourceCount = filteredData
                  .filter((item) => item.incomeSource === incomeSource)
                  .reduce((sum, item) => sum + item.households, 0);
                ...
                const percentage =
                  totalCount > 0
                    ? ((incomeSourceCount / totalCount) * 100).toFixed(1)
                    : "0";

                // Only show top 3 income sources
                ...
                return (
                  <Badge
                    ...
                    className="px-2 py-1"
                  >
                    {getIncomeSourceDisplayName(incomeSource)}: {percentage}%
                  </Badge>
                );
              })}
            </div>
