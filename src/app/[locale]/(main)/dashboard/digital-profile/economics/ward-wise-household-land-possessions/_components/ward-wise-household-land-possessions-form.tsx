...
interface WardWiseHouseholdLandPossessionsFormProps {
  editId: string | null;
  onClose: () => void;
  existingData: any[];
}

export default function WardWiseHouseholdLandPossessionsForm({
  editId,
  onClose,
  existingData,
}: WardWiseHouseholdLandPossessionsFormProps) {
  ...
  const addMutation =
    api.profile.economics.wardWiseHouseholdLandPossessions.create.useMutation({
      onSuccess: () => {
        toast.success(
          "नयाँ वडा अनुसार घरधुरी जग्गा स्वामित्व डाटा सफलतापूर्वक थपियो",
        );
  ...
  const updateMutation =
    api.profile.economics.wardWiseHouseholdLandPossessions.update.useMutation({
      onSuccess: () => {
        toast.success("वडा अनुसार घरधुरी जग्गा स्वामित्व डाटा अपडेट गरियो");
        utils.profile.economics.wardWiseHouseholdLandPossessions.getAll.invalidate();
        setIsSubmitting(false);
        onClose();
      },
  ...
  useEffect(() => {
    ...
    if (watchWardId && !editId) {
      ...
      if (duplicate) {
        ...
        setDuplicateError(
          `वडा ${wardNumber} को जग्गा स्वामित्व डाटा पहिले नै अवस्थित छ`,
        );
      }
    }
            ...
            <div className="bg-muted/40 p-4 rounded-lg">
              ...
              <div className="grid grid-cols-1 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>जग्गा भएका घरधुरी संख्या</FormLabel>
                      <FormControl>
                        <Input
                          type="number"
                          placeholder="0"
