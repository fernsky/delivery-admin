...
type WardWiseMajorOccupationData = {
  id: string;
  wardNumber: number;
  occupation: string;
  population: number;
...
interface WardWiseMajorOccupationTableProps {
  data: WardWiseMajorOccupationData[];
  onEdit: (id: string) => void;
}

export default function WardWiseMajorOccupationTable({
  data,
  onEdit,
}: WardWiseMajorOccupationTableProps) {
  ...
  const handleDelete = () => {
    if (deleteId) {
      deleteWardWiseMajorOccupation.mutate({
        id: deleteId,
      });
      setDeleteId(null);
    }
  ...
  // Calculate unique occupations for filtering
  ...
  const filteredData = data.filter((item) => {
    return (
      ...
      (filterOccupation === "all" || item.occupation === filterOccupation)
    );
  });
  ...
  const groupedByWard = filteredData.reduce(
    ...
    {} as Record<
      ...
      {
        wardNumber: number;
        items: WardWiseMajorOccupationData[];
            ...
            <div className="flex flex-col space-y-2 flex-grow">
              ...
              >
                <SelectTrigger className="w-full sm:w-[240px]">
                  <SelectValue placeholder="सबै पेशाहरू" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">सबै पेशाहरू</SelectItem>
        ...
        {sortedWardGroups.length === 0 ? (
          ...
          <div className="space-y-4">
            {viewMode === "list" ? (
              ...
              // Grid view - Occupations as rows and wards as columns
              <div className="overflow-x-auto">
                <Table className="min-w-max">
                  ...
                  <TableBody>
                    {uniqueOccupations.map((occupation) => {
                      ...
                      const totalOccupationPopulation = occupationData.reduce(
                        (sum, item) => sum + (item.population || 0),
                        0,
                      );

                      if (occupationData.length === 0) return null;

                      return (
                        <TableRow key={`grid-${occupation}`}>
                          ...
                          {sortedWardGroups.map((wardGroup) => {
                            const item = wardGroup.items.find(
                              (i) => i.occupation === occupation,
                            );

                            return (
                              <TableCell
                                key={`${wardGroup.wardNumber}-${occupation}`}
                          ...
                          {/* Total occupation population */}
                          <TableCell className="text-right font-medium">
                            {totalOccupationPopulation > 0
                              ? totalOccupationPopulation.toLocaleString()
                              : "-"}
                          </TableCell>
                        </TableRow>
                      );
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              वडा अनुसार प्रमुख पेशा डाटा मेट्ने?
            </AlertDialogTitle>
            <AlertDialogDescription>
