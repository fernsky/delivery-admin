...
type WardAgeGenderWiseMarriedAgeData = {
  id: string;
  wardId: string;
  wardNumber?: number;
  ageGroup: string;
...
interface WardAgeGenderWiseMarriedAgeTableProps {
  data: WardAgeGenderWiseMarriedAgeData[];
...
export default function WardAgeGenderWiseMarriedAgeTable({
  data,
  onEdit,
}: WardAgeGenderWiseMarriedAgeTableProps) {
  ...
  const deleteWardAgeGenderWiseMarriedAge =
    api.profile.demographics.wardAgeGenderWiseMarriedAge.delete.useMutation({
      onSuccess: () => {
  ...
  const handleDelete = () => {
    if (deleteId) {
      deleteWardAgeGenderWiseMarriedAge.mutate({ id: deleteId });
      setDeleteId(null);
    }
  };
  ...
  const groupedByWard = filteredData.reduce(
    ...
    {} as Record<
      string,
      {
        wardId: string;
        wardNumber: number;
        items: WardAgeGenderWiseMarriedAgeData[];
      }
    >,
  );
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              वडा अनुसार उमेर, लिङ्ग र विवाह उमेर डाटा मेट्ने?
            </AlertDialogTitle>
            <AlertDialogDescription>
