...
type WardDemographicSummaryData = {
  id: string;
  wardNumber: number;
...
interface WardDemographicSummaryTableProps {
  data: WardDemographicSummaryData[];
  onEdit: (id: string) => void;
}

export default function WardDemographicSummaryTable({
  data,
  onEdit,
}: WardDemographicSummaryTableProps) {
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [filterWard, setFilterWard] = useState<string>("all");

  const utils = api.useContext();
  const deleteWardDemographicSummary =
  ...
  const handleDelete = () => {
    if (deleteId) {
      deleteWardDemographicSummary.mutate({ id: deleteId });
      setDeleteId(null);
    }
  };
