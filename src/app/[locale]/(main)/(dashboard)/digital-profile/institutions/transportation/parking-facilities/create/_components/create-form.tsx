...
export function CreateParkingFacilityForm() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      description: "",
      type: undefined,
      wardNumber: parseInt("") || 0,
      location: "",
      address: "",
      areaInSquareMeters: parseFloat("") || 0,
      vehicleCapacity: parseFloat("") || 0,
      condition: undefined,
  ...
  const { mutate: createParkingFacility, isLoading } =
    api.profile.transportation.parkingFacilities.create.useMutation({
      onSuccess: (data) => {
        ...
        router.push(
          `/digital-profile/institutions/transportation/parking-facilities/${data.id}`,
        );
      },
      onError: (error) => {
