...
interface WardAgeGenderWiseEconomicallyActivePopulationTableProps {
  data: WardAgeGenderWiseEconomicallyActivePopulationData[];
...
const getAgeGroupDisplayName = (ageGroup: string): string => {
  switch (ageGroup) {
    case "AGE_0_TO_14":
      return "० देखि १४ वर्ष";
    case "AGE_15_TO_59":
      return "१५ देखि ५९ वर्ष";
    case "AGE_60_PLUS":
      return "६० वर्ष वा सोभन्दा बढी";
    default:
      return ageGroup;
  }
};
...
export default function WardAgeGenderWiseEconomicallyActivePopulationTable({
  data,
  onEdit,
}: WardAgeGenderWiseEconomicallyActivePopulationTableProps) {
  ...
  const deleteWardAgeGenderWiseEconomicallyActivePopulation =
    api.profile.economics.wardAgeGenderWiseEconomicallyActivePopulation.delete.useMutation(
      {
        onSuccess: () => {
  ...
  const handleDelete = () => {
    if (deleteId) {
      deleteWardAgeGenderWiseEconomicallyActivePopulation.mutate({
        id: deleteId,
      });
      setDeleteId(null);
    }
  ...
  // Calculate unique genders for filtering
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex flex-col space-y-2 flex-grow">
              ...
              <Select value={filterGender} onValueChange={setFilterGender}>
                <SelectTrigger className="w-full sm:w-[180px]">
                  <SelectValue placeholder="सबै लिङ्गहरू" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">सबै लिङ्गहरू</SelectItem>
        ...
        {sortedWardGroups.length === 0 ? (
          ...
          <div className="space-y-4">
            {viewMode === "list" ? (
                            ...
                            {wardGroup.items.map((item) => (
                              ...
                              >
                                <TableCell className="max-w-[240px] truncate">
                                  {getAgeGroupDisplayName(item.ageGroup)}
                                </TableCell>
                                <TableCell className="max-w-[120px] truncate">
                                  {getGenderDisplayName(item.gender)}
                                </TableCell>
              ...
              // Grid view - Age groups as columns, genders as rows
              <div className="overflow-x-auto">
                <Table className="min-w-max">
                  <TableHeader>
                    <TableRow>
                      ...
                      {/* Generate column headers for each age group */}
                      {uniqueAgeGroups.map((ageGroup) => (
                        <TableHead
                          key={ageGroup}
                  ...
                  <TableBody>
                    {uniqueGenders.map((gender) => {
                      // For each gender, create a row per ward
                      return sortedWardGroups
                        .map((wardGroup) => {
                          const genderData = wardGroup.items.filter(
                            (item) => item.gender === gender,
                          );

                          const totalGenderPopulation = genderData.reduce(
                            (sum, item) => sum + (item.population || 0),
                            0,
                          );

                          // Create a mapping of age group to item for this ward and gender
                          const ageGroupMap = genderData.reduce(
                            (map, item) => {
                              map[item.ageGroup] = item;
                              return map;
                            },
                            {} as Record<
                              string,
                          ...
                          return (
                            ...
                            >
                              ...
                              {/* Render cells for each age group */}
                              {uniqueAgeGroups.map((ageGroup) => {
                              ...
                              {/* Total ward gender population */}
                              <TableCell className="text-right font-medium">
                                {totalGenderPopulation > 0
                                  ? totalGenderPopulation.toLocaleString()
                                  : "-"}
                              </TableCell>
                            </TableRow>
                          );
                    ...
                    {/* Summary row with totals by age group */}
                    <TableRow className="bg-muted/20 font-medium">
                      <TableCell className="sticky left-0 bg-muted/20 z-10">
                        कुल जम्मा
                      </TableCell>
                      {uniqueAgeGroups.map((ageGroup) => {
                        const ageGroupTotal = filteredData
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              वडा अनुसार उमेर, लिङ्ग र आर्थिक रुपमा सक्रिय जनसंख्या डाटा मेट्ने?
            </AlertDialogTitle>
            <AlertDialogDescription>
